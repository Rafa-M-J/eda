...
server <- function(input, output, session) {
  ...
  highlight_extremes <- function(df, value_col, top_n){
    ...
    df %>%
      dplyr::group_by(연도) %>%
      dplyr::mutate(
        rank_desc = dplyr::dense_rank(dplyr::desc(!!v)),
        rank_asc  = dplyr::dense_rank(!!v),               
        color_group = dplyr::case_when(
          rank_desc <= k_top    ~ "top",     
          rank_asc  <= k_bottom ~ "bottom",  
          TRUE                  ~ "other"
        )
      ) %>%
      dplyr::ungroup()
  ...
  top_cost_by_year <- reactive({
    ...
    df %>%
      ...
      left_join(disease_info, by = "질병분류_22") %>%
      group_by(연도) %>%                                  
      arrange(desc(일인당진료비), .by_group = TRUE) %>%  
    ...
    ggplot(df, 
      ...
      geom_text(
        aes(
          ...
          hjust = ifelse(patient_growth_ratio >= 0, -0.4, 1.4)  
        ),
        size     = 3.5,
        fontface = "bold"
      ) +
