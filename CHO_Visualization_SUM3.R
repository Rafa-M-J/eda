...
ggplot(plot_data, 
  ...
  labs(
    ...
    subtitle = "환자 수는 22.6%지만, 입원 기간은 전체의 44.3%를 차지", #장기입원 심각
    x = NULL,
    y = "총 입내원일수"
  ) +
...
ggplot(patient_count_data, 
  ...
  labs(
    ...
    subtitle = "75세 이상 환자는 전체의 약 22.6%를 차지", 
    x = NULL,
    y = "총 진료실인원수"
  ) +
  theme_minimal(base_family = "NanumGothic") +
...
donut_lv <- levels(donut_df$disease_group)

donut_cols <- RColorBrewer::brewer.pal(max(3, length(donut_lv)), "Set3")[seq_along(donut_lv)]
names(donut_cols) <- donut_lv

if ("기타 (Others)" %in% names(donut_cols)) donut_cols["기타 (Others)"] <- "#C9C9C9"

p_donut <- ggplot(donut_df, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=disease_group)) +
  geom_rect(alpha=1) +                              
  coord_polar(theta="y") +
...
missing_codes <- setdiff(codes_pretty, names(code_cols))
if (length(missing_codes) > 0) {
  set3_all <- RColorBrewer::brewer.pal(12, "Set3")
  unused <- setdiff(set3_all, unname(donut_cols))
  if (length(unused) < length(missing_codes)) {
    unused <- c(unused, RColorBrewer::brewer.pal(8, "Dark2"))
  }
  code_cols <- c(code_cols, setNames(unused[seq_along(missing_codes)], missing_codes))
}

p_pretty_list <- ggplot(list_df, aes(x=1, y=reorder(Rank, -Rank))) +
  geom_tile(aes(fill = as.factor(Rank)), width=0.9, height=0.8, alpha=0.2, show.legend=FALSE) +
  geom_point(aes(x=0.6, color = Disease_Code), size=18, show.legend=FALSE, alpha=1) + # ★ alpha=1
  ...
  scale_color_manual(values = code_cols, drop = FALSE) +          
  scale_fill_brewer(palette="Set3") +
  xlim(0.5, 1.4) +
  labs(title="Top 10 저비용 질병 목록", subtitle="1인당 평균 진료비가 가장 낮은 순서 (2008-2023 종합)", x=NULL, y=NULL) +
  theme_void(base_family="NanumGothic")+ theme(
